<template>
  <div id="container">
    ...
    <component :is="currentVie" :contact="contact"></component>
...
export default {
  name: 'app',
  components: {
    ContactList, AddContact, UpdatePhoto, UpdateContact
  },
  ...
  methods:{
    ...
    addContact: function(contact){
      ...
      .then((response)=>{
        if(response.data.status === "success"){
          ...
          window.console.log("연락처 추가 실패 : " + response.data.message);
        }
      })
      .catch((ex)=>{
        window.console.log(" add Contact Failed : ", ex);
      })
    },
    updateContact: function(contact){
      ...
      .then((response)=>{
        if(response.data.status === "success"){
          ...
          window.console.log("연락처 변경 실패:" + response.data.message);
        }
      }).catch((ex)=>{
        window.console.log('updateContact failed : ', ex);
      })
    },
    fetchContactOne: function(no){
      ...
      .then((response)=>{
        ...
        window.console.log('fetchContactOne Failed', ex);
      })
    },
    deleteContact: function(no){
      ...
      .then((response)=>{
        if(response.data.status === "success"){
          ...
          window.console.log('연락처 삭제 실패 :' + response.data.message);
        }
      }).catch((ex)=>{
        window.console.log('delete failed', ex);
      })
    },
    updatePhoto : function(no, file){
      ...
      .then((response)=>{
        if(response.data.status === "success"){
          ...
          window.console.log('연락처 사진 변경 실패 : '+response.data.message);
        }
      }).catch((ex)=>{
        window.console.log('updatePhoto failed : ', ex);
      })
    }
  }
}
